...
class TestAIGenerator:
    ...
    def test_tool_execution_flow(self, mock_anthropic_class):
        ...
        mock_second_response.content = [Mock(text="Here's what I found about Python basics...")]
        
        # Third response would be final, but shouldn't be called since second has stop_reason="end"
        mock_client.messages.create.side_effect = [mock_tool_response, mock_second_response]
        ...
        assert len(messages) == 3  # Original user + assistant with tool + user with results
        assert messages[1]["role"] == "assistant"  # AI's tool use
        assert messages[2]["role"] == "user"  # Tool results
        assert messages[2]["content"][0]["type"] == "tool_result"
        assert messages[2]["content"][0]["content"] == "Python is a programming language..."
    ...
    def test_tool_execution_error_handling(self, mock_anthropic_class):
        ...
        mock_final = Mock()
        mock_final.stop_reason = "end"
        mock_final.content = [Mock(text="I encountered an error but here's what I can tell you...")]
